...
export function hasTimeConflict(sectionA: SectionWithTime, sectionB: SectionWithTime): boolean {
...
export async function detectScheduleConflicts(sectionIds: string[]): Promise<{
  hasConflicts: boolean;
  conflicts: Array<{ sectionA: SectionWithTime; sectionB: SectionWithTime }>;
}> {
  ...
  const sectionsWithTime: SectionWithTime[] = sections.map(s => ({
    id: s.id,
    courseId: s.courseId,
    days: s.days,
    time: s.time,
  ...
  const conflicts: Array<{ sectionA: SectionWithTime; sectionB: SectionWithTime }> = [];
...
export async function getTeacherSchedule(teacherId: string, sessionId?: string) {
...
export async function getStudentSchedule(studentId: string, sessionId?: string) {
  const where: any = {
    studentId,
    status: 'Active'
  };
...
export async function generateStudentSchedule(
  studentId: string,
  ...
  try {
    ...
    const sectionsByCourse = new Map<string, typeof availableSections>();
    ...
    const unavailableCourses = requestedCourseIds.filter(
      courseId => !sectionsByCourse.has(courseId) || sectionsByCourse.get(courseId)!.length === 0
    );
    ...
    const schedule = findValidSchedule(
      Array.from(sectionsByCourse.values()),
      [],
      0
    );
...
function findValidSchedule(
  ...
  for (const section of currentCourseSections) {
    ...
    if (!hasConflict) {
      ...
      const result = findValidSchedule(
        courseGroups,
...
export async function checkTeacherAvailability(
  ...
  for (const section of teacherSections) {
    const sectionData: SectionWithTime = {
      id: '',
      courseId: '',
      days: section.days,
      time: section.time,
    ...
    const newSectionData: SectionWithTime = {
      id: '',
      courseId: '',
      days: timeSlot.days,
      time: timeSlot.time,
      period: timeSlot.period || null
    };
