...
import type { Application, ProspectiveStudent } from '../generated/prisma';

export class EnrollmentWorkflowService {
  /**
   * Process accepted application and create student record
   */
  static async processAcceptedApplication(applicationId: string): Promise<void> {
    try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (application.prospectiveStudent.email) {
          ...
          await this.sendAcceptanceEmail(
            application.prospectiveStudent,
   ...
   */
  static async sendParentAccountEmail(
    email: string,
    name: string,
    tempPassword: string
  ): Promise<void> {
